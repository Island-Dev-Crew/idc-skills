...
- **Duplicate home**: the same fact, or a hand-maintained second entry file (a `CLAUDE.md` and an `AGENTS.md` both editing the map), lives in two places and the copies have diverged. One home per fact; a link beats a copy, so pick the home, make the other a pointer.
...
6. **Token check**: entry file plus one contract plus its inputs should land in roughly 2k to 8k tokens.
