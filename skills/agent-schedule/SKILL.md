...
Universal floor: cron granularity is **1 minute** (5-field expression, no seconds) — every runtime. For sub-minute cadence use a `while …; sleep N; done` loop or an event hook, never cron. Never put an LLM on a tight timer — cost and rate limits punish it.
