...
The text scan is a **whole-file, multiline-aware** static match (run by `python3`, which is **required** — if it is absent the text rung fails **closed**, never open), so a fetching context split across lines (`src` and its `="//h"` on separate lines, a `fetch(` whose URL literal is on the next line) and every `srcset` candidate are caught — a per-line regex structurally cannot do this. Its pass/fail counts travel on a **separate trusted channel** (a file only the scanner writes, strictly validated as three all-digit fields, anything else failing closed) — never parsed out of the findings stream, so a crafted finding byte (e.g. a NUL late in an otherwise-text file) cannot collapse the gate into a false green; finding lines are display-only with control bytes sanitized. It catches, case-insensitively so `HTTPS://` can't bypass: absolute schemes `http(s)://` **and** `wss://` / `ws://` / `ftp://` — including the backslash spellings (`https:\\h`, `https:\/\/h`) URL parsers normalize to slashes — and WebRTC `stun:` / `turn(s):`; the runtime tokens `fetch(`, `new WebSocket`, `XMLHttpRequest.open`, `EventSource`, `navigator.sendBeacon`, `new Worker` / `SharedWorker` / `importScripts(`, `serviceWorker.register(`, a dynamic `import(` of a URL, a **static** `import … from "//h"` / bare `import "//h"` / `export … from "//h"`, a `location` navigation (`location.assign/replace(`, `location.href =`, `location =`), and a dot-prefixed fetching **property** assignment (`img.src = "//h"`) — quoted with `'`, `"`, or a backtick template; `@import` / `url()` / `image-set()` of a URL in CSS; and protocol-relative `//host` (or backslash `\\host`, or an IPv6 literal `//[…]`) in every fetching attribute — `src`, every `srcset`/`imagesrcset` candidate, `href`/`xlink:href`, `poster`, `action`/`formaction`, `background`, `cite`, `ping`, `manifest`, `<object data>`, and a `url=` redirect — including a value with leading whitespace inside the quotes. Attribute matching is **tag-bounded**: a fetching attribute counts only inside a real `<tag …>` span, so a plain JS variable `const src='//docs'` is not egress, and `data=` fetches only on `<object>` (a `<div data='//docs'>` is inert). A genuine XML namespace declaration (`xmlns=`/`xmlns:pfx=` inside a tag — every one in the tag, not just the first) is the one non-fetching URI suppressed, and only there: a bare `xmlns=` in JS source, or a `fetch(xmlns=…)`, is real egress. Same-origin, `data:`, and `blob:` targets are not flagged. An intentional call is waived only by a **bounded, case-sensitive, trailing** `# egress-ok` / `// egress-ok` / `/* egress-ok */` comment on that hit's line, validated against a start-or-whitespace prefix and an end-of-line anchor, never a substring. Where the static match ever slips, the enforced rung 2 (sealed load) still catches the egress.
