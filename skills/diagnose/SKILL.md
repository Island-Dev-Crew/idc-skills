...
Generate **3 to 5 ranked, falsifiable hypotheses before testing any** (single-hypothesis generation anchors on the first plausible idea). Each states its prediction: *"If X is the cause, then changing Y makes the bug disappear."* No prediction = a vibe; discard or sharpen it. Show the ranked list to the user before testing (domain knowledge re-ranks instantly, "we just deployed #3"), but don't block if they're AFK.
