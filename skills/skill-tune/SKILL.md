...
The one finding that anchors the whole method: **the most efficient context files are small (roughly 500 to 800 tokens), and it is better to split a big one than to pad it.** Separating thought is what lets the model work; a tuned file is usually a *shorter* file.
...
Enforced-vs-advisory: in this island the **score-must-rise rule is procedural and advisory**: `results.tsv` is evidence, but no bundled validator recomputes its scores, pins its inputs, or mechanically reverts a losing edit. Call it `enforced` only in a repo that wires a red-capable validator to those exact artifacts and blocks the change on failure. Whether an over-cap trade-off is justified, and *which* edit to attempt (§3), remain advisory judgement even then. Say which; never let a hand-edited TSV impersonate a gate.
