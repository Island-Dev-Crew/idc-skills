...
A skill is prose the agent *follows*, so its instructions are the attack surface. The scan cannot judge intent — read the full `SKILL.md` and every bundled file for:
