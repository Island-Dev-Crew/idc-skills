...
| **AI-native** (`parent.ai`) | agent-facing endpoints and specs; agents **consume** it | `name.parent.ai`; APIs always at `api.*` |
...
Verify the exact required values against the current Vercel dashboard before entering them, because platforms change IPs. Entering them in the Namecheap/Vercel dashboards is a human-only dashboard trip: hand that to the [`wizard`](../wizard/SKILL.md) island, which opens each page and captures what's needed.
