...
Every other writing island *produces* text. This one *judges* it: a bundled scorer reads prose and returns a **0-100 AI-likeness score** — higher means more machine-extruded. The number is the point. "I made it sound human" is a claim; a score that fell from 67 to 6 is evidence from a check that could have failed. Model-agnostic: the tells (buzzword vocabulary, uniform sentence rhythm, chatbot residue, inflated significance) are shared across model families, so no vendor or model name is hardcoded.
