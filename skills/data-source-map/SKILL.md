...
A data source is a leaf in the workspace, reached from the map's routing table like any room. Name it `data-<name>.md` when the source is a node in a map (the typed-file convention), or make it the leaf folder's `CONTEXT.md` when the source carries its own references or sub-structure. Either way it is a descriptor, not a dump:

```markdown
---
type: data-asset
kind: postgres        # postgres | bigquery | oracle | sqlite | google-sheet | drive | sharepoint
freshness: live       # live | nightly | snapshot:<date>
access: read-only
---
# Data source: <name>
...
The frontmatter makes the source queryable and indexable: a dashboard or a generated file-map can list every data source by kind and freshness without opening one. The **Ask it** line is the whole point, the *request slip* in the library analogy: it saves the agent from ripping the schema apart to find where orders are. For a big warehouse you don't describe every table; you describe *where to look*, so a 10,000-table Oracle store becomes navigable from a paragraph.

## Wire it in

1. **Describe, don't ingest.** Write the descriptor from what the operator knows about their own data (they know it better than any crawler). Do not copy the data into markdown. Point at it. A stage or process that uses this source names it once, here, and links to it (`consumes: [[data-<name>]]`) rather than restating its reach: one home per fact. The descriptor is the home of the data-asset node; the contracts that read it are edges into it.
...
**Done when** the descriptor states kind / reach / holds / ask-it / freshness / do-not, the credential is referenced (never inlined), the reach is read-only, and every table, join, or path the "Ask it" line names has been exercised (not merely confirmed to exist) by a query or listing, with `Freshness` confirmed or marked unverifiable, or the whole line is marked `[UNVERIFIED]`.
